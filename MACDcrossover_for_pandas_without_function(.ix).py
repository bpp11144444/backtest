...
# 4 means'Close', 13 means 'macd', 14 means 'macdsig', 16 means 'amount', 17 means 'shares',18 means'total'

for i in range(1, len(df)):
        df.iloc[i, 17]=df.iloc[i-1,17]
        df.iloc[i, 16]=df.iloc[i-1,16]
        df.iloc[i, 18]=df.iloc[i, 17]*df.iloc[i,4]+df.iloc[i,16]
        if df.iloc[i,17] == 0 and df.iloc[i,14] < df.iloc[i,13]:
            df.iloc[i,17] = df.iloc[i,16]/df.iloc[i,4]
            df.iloc[i,16] = df.iloc[i,16]-df.iloc[i,17]*df.iloc[i,4]
            trade_time+=1
        elif df.iloc[i,17] > 0 and df.iloc[i,14] >= df.iloc[i,13]:
            df.iloc[i, 16]=df.iloc[i,17]*df.iloc[i,4]
            df.iloc[i,17]=0
            trade_time += 1
        else:
            continue
